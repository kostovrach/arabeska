...
export default defineEventHandler(
    async (event): Promise<{ status: number; message?: string; success: boolean }> => {
        ...
        const { product_id, modifier } = await readBody<ICartItem>(event);
        if (!product_id || !modifier)
            return { status: 400, message: 'Missing required parameters', success: false };
        ...
        try {
            ...
            if (existingIdx !== -1) {
                ...
                if (!res) return { status: 404, message: 'User not found', success: false };
